...
interface TournamentCommanderBreakdown {
  commanders: Array<{
    // Commanders used in the tournament
...
interface TournamentColorBreakdown {
  colors: Array<{
    // Individual colors in the tournament
...
const standing = await TournamentResultsService.getStandingById(
  tournamentId,
  standingId
);
...
const deckTournaments = await DeckTournamentService.getDeckTournamentHistory(
  deckId
);

// 4. Get tournament results for a player
const playerId = "player101";
const playerStandings = await TournamentResultsService.getStandingsForPlayer(
  playerId
);
```
