-- Update the card commander detail stats function to include commander's top_cuts and expected_top_cuts
-- These are needed to calculate the commander's conversion score for delta comparison
...
CREATE OR REPLACE FUNCTION get_card_commander_detail_stats(
  ...
  commander_total_top_cuts BIGINT,
  commander_total_expected_top_cuts DOUBLE PRECISION
) AS $$
BEGIN
  RETURN QUERY
  WITH card_weekly AS (
  ...
  SELECT
    ...
    ct.total_top_cuts AS commander_total_top_cuts,
    ct.total_expected_top_cuts AS commander_total_expected_top_cuts
  FROM card_weekly cw
...
COMMENT ON FUNCTION get_card_commander_detail_stats IS 'Efficiently aggregates weekly card stats for a specific commander with weekly and total commander baseline data including conversion score metrics';
