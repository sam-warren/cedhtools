...
import { cardColumns, type DeckCard } from "../../lib/components/commanders/card-columns";
import { deckColumns } from "../../lib/components/commanders/deck-columns";
import { Deck } from "../../lib/components/commanders/deck-columns";
import WinRateChart from "@/src/lib/components/_molecules/charts/win-rate-chart";
import { SeatWinRateChart } from "@/src/lib/components/_molecules/charts/seat-win-rate-chart";
import { DataTable } from "@/components/ui/data-table";
import { PageHeader } from "@/src/lib/layout/page-header";
import PopularityChart from "@/src/lib/components/_molecules/charts/popularity-chart";
import { usePathname } from "next/navigation";

export default function CommanderPage(props: { params: Promise<{ commander_ids: string }> }) {
  const pathname = usePathname();
  const commanderIds = pathname && pathname.split(",").sort();
      ...
      <div>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground d-foreground mt-2 text-sm">{commanderData.totalGames} decks</p>
          </div>
        </div>
      </div>
      <Separator />
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-muted-foreground mt-1 text-xs">Out of 129 tournaments</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-muted-foreground mt-1 text-xs">Reliably high-performing deck</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            ...
            <p className="text-muted-foreground mt-1 text-xs">Not prone to drawing games</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            ...
            <p className="text-muted-foreground mt-1 text-xs">Regularly makes top cut</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-muted-foreground mt-1 text-xs">Very popular in the meta</p>
          </CardContent>
        </Card>
      </div>
