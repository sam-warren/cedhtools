...
import { CEDHToolsIcon } from "@/components/icons/cedhtools-icon";
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="w-full px-4">
        <div className="flex h-14 w-full items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <Button
              variant="outline"
              className="h-8 w-[200px] justify-start rounded-[0.5rem] bg-muted/50 text-sm font-normal text-muted-foreground sm:w-[300px]"
              onClick={() => setOpen(true)}>
              <span className="hidden sm:inline-flex">Search for a commander...</span>
              <span className="sm:hidden">Search...</span>
              <kbd className="ml-auto hidden h-5 items-center gap-1 rounded border bg-muted px-1.5 font-mono text-[10px] font-medium sm:flex">
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        ...
        <CommandList>
          ...
          <CommandGroup heading="Popular Commanders">
