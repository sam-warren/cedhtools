...
const CardNameCell = ({ row }: { row: Row<CommanderCard> }) => {
  ...
  return (
    <Link 
      href={`/commanders/${commanderId}/cards/${row.original.id}`}
...
export const columns: ColumnDef<CommanderCard>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div>{row.getValue("inclusion")}%</div>
    )
  },
  {
    accessorKey: "winRate",
    ...
    cell: ({ row }) => (
      <div>{row.getValue("winRate")}%</div>
    )
  },
  {
    accessorKey: "drawRate",
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title="Draw Rate" />
    ),
    cell: ({ row }) => (
      <div>{row.getValue("drawRate")}%</div>
    )
  }
];
