...
class ImageCacheService {
  ...
  private static BATCH_DELAY = 100;
  ...
  constructor() {
    this.worker = new Worker(new URL('../../workers/imageCache.worker.ts', import.meta.url), {
      type: 'module'
    });
  ...
  private async processQueue(): Promise<void> {
    ...
    try {
      while (this.loadingQueue.length > 0) {
        ...
        await Promise.all(
          batch.map(({ scryfall_id, imageUrl, resolve, reject }) => {
            return new Promise<void>((promiseResolve) => {
              this.callbacks.set(scryfall_id, { resolve, reject });
      ...
      // Handle any remaining callbacks in the batch
      this.callbacks.forEach(callback => {
        callback.reject(error);
      });
      this.callbacks.clear();
